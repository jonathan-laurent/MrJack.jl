...
function char_at_pos(arr)
    return g.board[arr[1]][arr[0]].character
end
        ...
        elseif s[2] == "user" && length(s) >= 4
            ...
            elseif s[3] == "power" && length(args) >= 1
                if g.selected == JEREMY_BERT
                    action = MoveLid(arr_to_tuple(args[0]["start"]), arr_to_tuple(args[0]["end"]))
                elseif g.selected == WILLIAM_GULL
                    character = char_at_pos(args[0]["end"])
                    if character == WILLIAM_GULL
                        character = char_at_pos(args[0]["start"])
                    end
                    if character !== nothing
                        action = SwapWilliamGull(character)
                    end
                elseif g.selected == JOHN_WATSON && length(args) == 1
                    action = ReorientWatsonLight(arr_to_tuple(args[0]["end"]))
                elseif g.selected == INSPECTOR_LESTRADE
                    action = MoveCops(arr_to_tuple(args[0]["start"]), arr_to_tuple(args[0]["end"]))
                elseif g.selected == MISS_STEALTHY && length(args) == 1
                    action = MoveCharacter(arr_to_tuple(args[0]["end"]))
                elseif g.selected == JOHN_SMITH
                    action = MoveLamp(arr_to_tuple(args[0]["start"]), arr_to_tuple(args[0]["end"]))
